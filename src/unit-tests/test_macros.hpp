...
/* Make a nice name for the standard version */
		...
		#define ACTUAL_CPP_VER _MSVC_LANG
	#else
		#define ACTUAL_CPP_VER __cplusplus
	#endif

	#if ACTUAL_CPP_VER <= 199711L
		#define TEST_STD_VER 3
	#elif ACTUAL_CPP_VER <= 201103L
		#define TEST_STD_VER 11
	#elif ACTUAL_CPP_VER <= 201402L
		#define TEST_STD_VER 14
	#elif ACTUAL_CPP_VER <= 201703L
		#define TEST_STD_VER 17
	#elif ACTUAL_CPP_VER <= 202002L
		#define TEST_STD_VER 20
	#elif ACTUAL_CPP_VER <= 202302L
...
#if TEST_HAS_FEATURE(address_sanitizer) || TEST_HAS_FEATURE(hwaddress_sanitizer) || TEST_HAS_FEATURE(memory_sanitizer) || TEST_HAS_FEATURE(thread_sanitizer)
...
namespace test_macros_detail {
	template<class v_type, class U> struct is_same {
		enum { value = 0 };
	};
	template<class v_type> struct is_same<v_type, v_type> {
		enum { value = 1 };
	};
}// namespace test_macros_detail

#define ASSERT_SAME_TYPE(...) static_assert((test_macros_detail::is_same<__VA_ARGS__>::value), "Types differ unexpectedly")
...
template<class Tp> inline Tp const& DoNotOptimize(Tp const& value) {
...
template<class Tp> inline Tp& DoNotOptimize(Tp& value) {
