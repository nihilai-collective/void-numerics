...
namespace VoidNumerics\Packager;
...
';
    ...
    function secondBuild(string $portFileContent): bool
    {
        ...
        $this->sudo('./vcpkg format-manifest ./ports/void-numerics/vcpkg.json');
        /* Note: We commit this in /usr/local, but we never push it (we can't) */
        $this->git('add .', true);
        $this->git('commit -m "VCPKG info update"', true);
        $this->sudo('/usr/local/share/vcpkg/vcpkg x-add-version void-numerics');

        echo GREEN . "Copy back port files from /usr/local/share...\n" . WHITE;
        chdir(getenv('HOME') . '/void-numerics');

        if (!is_dir('./vcpkg/versions/v-')) {
            mkdir('./vcpkg/versions/v-', 0755, true);
        }

        system('cp -v -R /usr/local/share/vcpkg/ports/void-numerics/vcpkg.json ./vcpkg/ports/void-numerics/vcpkg.json');
        system('cp -v -R /usr/local/share/vcpkg/versions/v-/void-numerics.json ./vcpkg/versions/v-/void-numerics.json');
        ...
        $resultCode = 0;
        $output = [];
        exec($this->sudo . ' /usr/local/share/vcpkg/vcpkg install void-numerics:x64-linux', $output, $resultCode);
